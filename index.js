...
const generatePasswordSection = document.querySelector(
...
const resetOrRegeneratePassword = document.querySelector(
  `.resetOrRegeneratePassword`
);
...
const checkboxConfirmPassword = document.querySelector(
...
const symbolsNumbers = [...Array(31)].map((_, i) =>
...
function generatePassword() {
  ...
  function generateFirsPartPassword() {
  ...
  counter = 0;
  const passwordAnimation = setInterval(() => {
    ...
    if (counter < userPasswordStrength(lengthPassword)) {
      ...
      circleContainer.style.background = `conic-gradient(red 0deg, orange ${
        counter * RATIO_TO_CONVERT_TO_DEGREES
      }deg, yellow ${counter * RATIO_TO_CONVERT_TO_DEGREES}deg, green ${
        counter * RATIO_TO_CONVERT_TO_DEGREES
      }deg, blue ${counter * RATIO_TO_CONVERT_TO_DEGREES}deg);`;
      // circleContainer.style.background = `conic-gradient(green ${counter*3.6}deg, gray ${counter*3.6}deg)`;
    } else {
      const shuffledPassword = shuffleArray(Array.from(firstPartpassword)).join(
        ""
      );
      function checkPassword() {
        if (usedPasswords.get(shuffledPassword)) {
          generateFirsPartPassword();
          checkPassword();
        } else {
          displayPassword.innerHTML = shuffledPassword;
          usedPasswords.set(shuffledPassword, true);
        }
      }
      checkPassword();
      //   if password exksts, re-run  `generateFirsPartPassword`

      clearInterval(passwordAnimation);
    }
  }, 75);
